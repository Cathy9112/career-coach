...
1. 只有【用户简历】中明确出现的内容才能作为候选人的真实事实。目标岗位、岗位JD、常见行业要求和你的专业常识都只能用于分析匹配度，绝不能转写成候选人已经具备的经历或能力。
2. 禁止新增、猜测、推断或合理化任何姓名、联系方式、年龄、所在地、公司、岗位、任职时间、项目名称、职责、技术栈、学历、学校、专业、成绩、证书、奖项、薪资、成果和量化数据。
3. 禁止把模糊描述擅自具体化。原文没有数字时不得添加人数、金额、比例、排名、耗时、次数、用户量、性能提升等数字；原文没有时间时不得补写年月；原文没有技能时不得仅因岗位JD要求而添加该技能。
...
2. 用户提供岗位JD时，必须优先提取其中的岗位职责、必备技能、经验要求和加分项，并逐项对照简历；未提供JD时才按目标岗位的通用要求分析。
...
3. JD适配：用户提供岗位JD时，必须优先使用JD中的职责、技能关键词和经验要求调整内容顺序与表达，不得把简历中不存在的JD要求改写成用户已经具备的经历。
...
def optimize_resume(resume_text: str, target_position: str, job_description: str = "") -> str:
    ...
    prompt = RESUME_OPTIMIZE_PROMPT.format(
        ...
        job_description=job_description or "未提供岗位JD，请按目标岗位通用要求分析。",
...
def generate_optimized_resume(resume_text: str, target_position: str, job_description: str = "") -> str:
    ...
    prompt = OPTIMIZED_RESUME_PROMPT.format(
        truthfulness_rules=TRUTHFULNESS_RULES,
        target_position=target_position,
        job_description=job_description or "未提供岗位JD，请按目标岗位通用要求优化。",
        resume_text=resume_text
    )
    messages = [{"role": "system", "content": prompt}]
    response = chat_completion(messages, stream=False)
